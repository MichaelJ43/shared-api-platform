...
- **`GET /v1/admin/analytics/events`** — admin only. Either **`appId` + `day=YYYY-MM-DD`** (optional `cursor` for pagination), **or** **`appId` + `from` + `to`** (ISO-8601 datetimes, UTC recommended) for a window up to **31 days**. Returns **`{ items, nextCursor }`** (`nextCursor` is null for range queries). Items include **`serverTimestamp`**, **`eventType`**, **`path`**, **`ingestId`**, etc.
