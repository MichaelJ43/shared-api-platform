...
- **`POST /v1/auth/register`** — JSON body: `email`, `password`, optional `returnUrl`. `email` must be a valid email address (trimmed/lowercased server-side). `password` must satisfy the server password policy: 12–128 characters with uppercase, lowercase, digit, and special character, and cannot equal the email or local part. Creates a user in DynamoDB (`auth_users`), stores only an Argon2id password hash, and starts a session (same cookie as login). **Registration is open only when both are true:** (1) Lambda env **`AUTH_ALLOW_REGISTER=true`** (Terraform `auth_allow_register`, infrastructure kill-switch), and (2) the **admin preference** in DynamoDB **`platform_settings`** (default off until an admin enables it on the dashboard **Users** page or via **`PATCH /v1/admin/site`**). Otherwise the endpoint returns **404** `not_found`. The **auth SPA** **`signup.html`** is the browser entry point when registration is open.
...
- **`GET /v1/admin/analytics/app-ids`** — admin only. Bounded **Scan** of the events table (projection `pk` only) to discover **appId** values for dashboards. May be incomplete on very large tables.
- **`GET /v1/admin/analytics/events`** — admin only. Either **`appId` + `day=YYYY-MM-DD`** (optional `cursor` for pagination), **or** **`appId` + `from` + `to`** (ISO-8601 datetimes, UTC recommended) for a window up to **31 days**. Returns **`{ items, nextCursor }`** (`nextCursor` is null for range queries). Items include **`serverTimestamp`**, **`eventType`**, **`path`**, **`ingestId`**, optional **`properties`** (ingest **`context`** from clients), etc. The **analytics dashboard** table includes a **Context** column (expandable JSON), client-side filters on **`eventType`** and **substring in `properties`**, and **Load more** for **`day`** queries when **`nextCursor`** is present. See **[dashboard context UI plan](dashboard-context-properties-plan.md)** for the full product note.
